...
class stimulator
{
    private:
        ...
        time_t start_time, end_time, move_time;
    public:
        stimulator()
        {
            int i,j;
         ...
         void check_n_update()
        {
            ...
            for(i=0;i<NO_OF_PLAYERS;i++)
            {
                ...
                do
                {
                    ...
                    if(place_reached==POWER_UP) 
                    {
                        printf("It gets a powerup");
                        fprintf(fp,"(Power Up)->");
                    }    
        ...
        void controller()
        {
            ...
            while(players_alive_on_board>1)
            {
                ...
                for(i=0;i<NO_OF_PLAYERS;i++)
                {
                    ...
                    if(move_time>=2)
                    {
                        printf("\nWarnig:player %c exceeded time limit and took %d sec.",p[i].id,move_time);
                    }     
                    /*
            ...
            for(i=0;i<NO_OF_PLAYERS;i++)
            {
                ...
                {
                     printf("\n player %c Knocked Out ----!.\n",p[i].id);
                    fprintf(fp,"%c -> Knocked Out ----!.\n",p[i].id);
                }
            }    
            fclose(fp); 
            display_board();           
